...
import { doc, collection, runTransaction, serverTimestamp, query, where, getDocs, writeBatch, increment } from 'firebase/firestore';
import { getAuth } from 'firebase/auth';
/**
 * Place a bet on a special event
 */
export async function placeEventBet(eventId, amount, prediction) {
    const uid = getAuth().currentUser?.uid;
    if (!uid)
        throw new Error("Must be signed in to place bet");
    if (amount <= 0)
    ...
    await runTransaction(db, async (transaction) => {
        ...
        const existingBetQuery = query(betsCollection, where("eventId", "==", eventId), where("userId", "==", uid));
 ...
 */
export async function getUserEventBets(userId) {
    const betsCollection = collection(db, 'eventBets');
 ...
 */
export async function resolveEventBets(eventId, outcome) {
    const betsCollection = collection(db, 'eventBets');
