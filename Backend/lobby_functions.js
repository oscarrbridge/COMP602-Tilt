 ...
 */
export async function joinGameLobby(gameId, uid, displayName) {
 ...
 */
export async function createGameLobby(hostUid, gameType = "blackjack", minPlayers = 2, maxPlayers = 6) {
 ...
 */
export async function deleteGameLobby(gameId) {
    try {
 ...
 */
export async function updatePlayerData(gameId, uid, updates) {
    const playerRef = doc(db, "games", gameId, "players", uid);
 ...
 */
export async function updatePokerRound(gameId, updates) {
    const gameRef = doc(db, "games", gameId);
    await updateDoc(gameRef, {
        ...updates,
        updatedAt: serverTimestamp(),
