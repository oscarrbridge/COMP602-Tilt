...
const sortCards = (cards) => cards.sort((a, b) => rankValues[b.rank] - rankValues[a.rank]);
...
export function evaluateHand(cards) {
    ...
    if (straightFlushHigh)
        return 8000000 + straightFlushHigh * 10;
    // Four of a Kind
    const quads = Object.keys(rankCount).find((k) => rankCount[+k] === 4);
    if (quads)
        return 7000000 + +quads * 10;
    // Full House
    ...
    // Flush
    if (flushCards)
        return 5000000 + rankValues[flushCards[0].rank];
    // approximate high card of flush
    // Straight
    if (straightHigh)
        return 4000000 + straightHigh * 10;
    // Three of a Kind
    if (trips.length >= 1)
        return 3000000 + trips[0] * 10;
    // Two Pair
    if (pairs.length >= 2)
        return 2000000 + pairs[0] * 100 + pairs[1] * 10;
    // One Pair
    if (pairs.length >= 1)
        return 1000000 + pairs[0] * 10;
    // High Card
