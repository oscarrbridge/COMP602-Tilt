...
const getStraightHigh = (cards: Card[]) => {
  ...
  if(values.includes(14)&&values.includes(5)&&values.includes(4)&&values.includes(3)&&values.includes(2)) return 5;
  return null;
}
...
export function evaluateHand(cards: Card[]): number {
  ...
  // Full House
  const trips = Object.keys(rankCount).filter(k=>rankCount[+k]===3).map(k=>+k).sort((a,b)=>b-a);
  const pairs = Object.keys(rankCount).filter(k=>rankCount[+k]===2).map(k=>+k).sort((a,b)=>b-a);
  if(trips.length>=1 && (pairs.length>=1 || trips.length>=2)) return 6000000 + trips[0]*100 + (pairs[0]||trips[1])*10;
  // Flush
  if(flushCards) return 5000000 + flushCards[0].rank as unknown as number; // approximate high card of flush
  // Straight
