...
export default function Roulette() {
    ...
    const handleResult = async (roll, stakeCents) => {
        // stakeCents already in cents
        let multiplier = 0;
        if (selectedBet === 'Red' && RED_SET.has(roll))
            multiplier = 2;
        if (selectedBet === 'Black' && roll !== 0 && !RED_SET.has(roll))
            multiplier = 2;
        if (selectedBet === 'Odd' && roll % 2 === 1)
            multiplier = 2;
        if (selectedBet === 'Even' && roll !== 0 && roll % 2 === 0)
            multiplier = 2;
        if (selectedBet === 'Number' && roll === chosenNumber)
            multiplier = 36;
        if (multiplier > 0) {
    ...
    return (_jsx(BackgroundLayout, { children: _jsx(CurrencyProvider, { base: 'NZD', DefaultCurrency: 'NZD', children: _jsx("div", { className: 'casino-container', children: _jsxs("div", { className: 'roulette-app', children: [_jsxs("div", { className: 'roulette-panel', children: [status === 'Idle' && (_jsx(BetControls, { balance: balance, bet: bet, setBet: setBet, startGame: startGame })), _jsxs("div", { className: 'betting-box', children: [_jsx("button", { className: `bet-btn ${selectedBet === 'Red' ? 'active' : ''}`, onClick: () => {
                                                setSelectedBet('Red');
                                                setChosenNumber(null); // clear custom number highlight
                                            }, disabled: status === 'Spinning', children: "Red" }), _jsx("button", { className: `bet-btn ${selectedBet === 'Black' ? 'active' : ''}`, onClick: () => {
                                                setSelectedBet('Black');
                                                setChosenNumber(null); // clear custom number highlight
                                            }, disabled: status === 'Spinning', children: "Black" }), _jsx("button", { className: `bet-btn ${selectedBet === 'Number' ? 'active' : ''}`, onClick: () => {
                                                setSelectedBet('Number');
                                                setChosenNumber(0);
                                            }, disabled: status === 'Spinning', children: "Custom" }), _jsx("button", { className: `bet-btn ${selectedBet === 'Odd' ? 'active' : ''}`, onClick: () => {
                                                setSelectedBet('Odd');
                                                setChosenNumber(null); // clear custom number highlight
                                            }, disabled: status === 'Spinning', children: "Odd" }), _jsx("button", { className: `bet-btn ${selectedBet === 'Even' ? 'active' : ''}`, onClick: () => {
                                                setSelectedBet('Even');
                                                setChosenNumber(null); // clear custom number highlight
                                            }, disabled: status === 'Spinning', children: "Even" }), _jsxs("div", { className: 'number-grid-wrapper', children: [_jsx("div", { className: 'zero-row', children: _jsx("div", { className: `number-item green zero ${chosenNumber === 0 ? 'active' : ''}`, onClick: () => {
                                                            setSelectedBet('Number');
                                                            setChosenNumber(0);
                                                        }, children: "0" }) }), _jsx("div", { className: 'number-grid', children: Array.from({ length: 36 }, (_, i) => {
                                                        const num = i + 1;
                                                        return (_jsx("div", { className: `number-item ${numberColors[num]} ${chosenNumber === num ? 'active' : ''}`, onClick: () => {
                                                                setSelectedBet('Number');
                                                                setChosenNumber(num);
                                                            }, children: num }, num));
                                                    }) })] })] })] }), _jsx("div", { className: 'wheel-area', children: _jsxs("div", { className: 'wheel-viewport', children: [_jsx("svg", { className: 'wheel-svg', viewBox: `0 0 ${size} ${size}`, width: size, height: size, "aria-hidden": false, children: _jsxs("g", { className: 'wheel-g', style: {
                                                transform: `rotate(${rotationDeg}deg)`,
                                                transformOrigin: `${cx}px ${cy}px`,
                                            }, children: [_jsx("circle", { cx: cx, cy: cy, r: rOuter + 6, className: 'rim' }), WHEEL_NUMBERS.map((num, i) => {
                                                    ...
                                                    return (_jsxs("g", { children: [_jsx("path", { d: pathD, className: `slice ${fillClass}` }), _jsx("text", { x: labelPos.x, y: labelPos.y, className: `slot-label ${fillClass}`, transform: `rotate(${centerAngle}, ${labelPos.x}, ${labelPos.y})`, children: num })] }, i));
                                                }), _jsx("circle", { cx: cx, cy: cy, r: rInner - 18, className: 'hub' }), _jsx("text", { x: cx, y: cy, className: 'hub-text', textAnchor: 'middle', dominantBaseline: 'middle', children: "ROULETTE" })] }) }), _jsx("div", { className: 'pointer', "aria-hidden": 'true' })] }) }), status == 'Result' && (_jsxs("div", { className: 'roulette-results', children: [resultNumber !== null && (_jsxs("span", { className: 'roulette-result', children: ["Result: ", resultNumber, ' ', resultNumber === 0 ? 'Green' : RED_SET.has(resultNumber) ? 'Red' : 'Black'] })), payout > 0 ? (_jsxs("span", { className: 'roulette-result win', children: ["You won: $", (payout / 100).toFixed(2)] })) : status === 'Result' ? (_jsx("span", { className: 'roulette-result lose', children: "No win" })) : null] }))] }) }) }) }));
}
